...
class Manager:
    ...
    def execute(self, query):
        ...
        return (yield from queries.execute(query))
    ...
    def prefetch(self, query, *subqueries):
        ...
        return (yield from queries.prefetch(query, *subqueries))
    ...
    def count(self, query, clear_limit=False):
        ...
        """
        query = self._swap_database(query)
        return (yield from queries.count(query, clear_limit=clear_limit))
    ...
    def scalar(self, query, as_tuple=False):
        ...
        """
        query = self._swap_database(query)
        return (yield from queries.scalar(query, as_tuple=as_tuple))
    ...
    def atomic(self):
        ...
        **asynchronous** context manager.
        ...
        """
        return transactions.atomic(self.database)

    def transaction(self):
        """Similar to `peewee.Database.transaction()` method, but returns
        **asynchronous** context manager.
        """
        return transactions.transaction(self.database)

    def savepoint(self, sid=None):
        """Similar to `peewee.Database.savepoint()` method, but returns
        **asynchronous** context manager.
        """
        return transactions.savepoint(self.database, sid=sid)

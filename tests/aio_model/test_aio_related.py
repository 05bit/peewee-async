...
@dbs_all
async def test_aio_fetch_not_cached(db: AioDatabase, mocker: MockFixture) -> None:
    ...
    loaded = await beta.aio_fetch("alpha")
...
@dbs_all
async def test_aio_fetch_via_field(db: AioDatabase, mocker: MockFixture) -> None:
    alpha = await TestModelAlpha.aio_create(text="Alpha")
    beta = await TestModelBeta.aio_create(alpha_id=alpha.id, text="Beta")

    loaded = await beta.aio_fetch(TestModelBeta.alpha)
    assert loaded.id == alpha.id
    assert loaded.text == alpha.text


@dbs_all
async def test_aio_fetch_error(db: AioDatabase) -> None:
    alpha = await TestModelAlpha.aio_create(text="Alpha")

    with pytest.raises(ValueError):
        await alpha.aio_fetch(TestModelAlpha.text)  # type: ignore


@dbs_all
async def test_aio_fetch_cached(db: AioDatabase, mocker: MockFixture) -> None:
    alpha = await TestModelAlpha.aio_create(text="Alpha")
...
@dbs_all
async def test_aio_fetch_cached_via_join(db: AioDatabase, mocker: MockFixture) -> None:
    ...
    spy_get = mocker.spy(TestModelAlpha, "get")
    loaded = await result.aio_fetch("alpha")
    assert loaded.id == alpha.id
    spy_get.assert_not_called()


@dbs_all
async def test_aio_fetch_null(db: AioDatabase, mocker: MockFixture) -> None:
...
@dbs_all
async def test_aio_fetch_lazy_load(db: AioDatabase, mocker: MockFixture) -> None:
    alpha = await TestModelAlpha.aio_create(text="Alpha")
    beta = await TestModelBeta.aio_create(alpha_id=alpha.id, text="Beta")

    TestModelBeta.alpha.lazy_load = False
    with pytest.raises(ValueError):
        await beta.aio_fetch(TestModelBeta.alpha)
